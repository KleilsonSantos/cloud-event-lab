...
Follow [`AGENTS.md`](../../AGENTS.md) — including **Owner Cadence**. Prefer the smallest slice that advances the current phase.

## Mission

Given a goal, propose a short plan. Obey cadence:

- `next` → proposal only (this format); do not code
- `ok` / `prossegue` → implement via Issue → branch → PR → `sandbox`
- `green` → only then promote `sandbox` → `main`
...
- [ ] Does not treat `ok` as `green`

## Response format (`next`)
